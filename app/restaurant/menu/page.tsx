...
type CompForm = { id?: number; name: string; price: string; stock: string; disponible: boolean };
const compLabel = (k: Complement) => (k.disponible === false ? "Indisponible" : k.stock === null || k.stock === undefined ? null : k.stock <= 0 ? "Épuisé" : `Reste ${k.stock}`);
...
export default function MenuPage() {
  ...
  const toggleCatVisible = async (c: Categorie) => {
    const visible = c.visible === false;
    setCats((cs) => cs.map((x) => x.id === c.id ? { ...x, visible } : x));
    try { await restaurateurService.setCategorieVisible(c.id, visible); toast.success(visible ? `« ${c.name} » visible par les clients` : `« ${c.name} » masquée : ses plats ne sont plus commandables`); }
  ...
  const saveComp = async () => {
    ...
    setSaving(true);
    try {
      const st = compForm.stock.trim();
      if (st !== "" && !/^\d+$/.test(st)) { toast.error("La quantité du jour doit être un nombre entier, ou vide pour illimité."); setSaving(false); return; }
      const extra = { stock: st === "" ? "" : Number(st), disponible: compForm.disponible };
      if (compForm.id) await restaurateurService.updateComplement(compForm.id, { name: compForm.name.trim(), price, ...extra });
      else await restaurateurService.createComplement({ name: compForm.name.trim(), price, restaurantId, ...extra });
    ...
    <Tabs defaultValue="plats" className="space-y-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">{p.categorieId != null ? catName[p.categorieId] : "Sans catégorie"}{p.categorieId != null && catHidden.has(p.categorieId) ? <span className="ml-1 rounded bg-muted px-1 font-semibold">catégorie masquée</span> : null}</div>
                    </div>
      ...
      <TabsContent value="complements" className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button className="" onClick={() => setCompForm({ name: "", price: "", stock: "", disponible: true })}><IconPlus className="mr-2 h-4 w-4" />Nouveau complément</Button>
        </div>
        <div className="divide-y rounded-lg border bg-card">
          {comps.map((k) => (
            <div key={k.id} className={`flex items-center gap-3 px-3 py-2 ${k.disponible === false || (k.stock !== null && k.stock !== undefined && k.stock <= 0) ? "opacity-60" : ""}`}>
              <span className="flex-1 font-medium">{k.name}{compLabel(k) && <span className={`ml-2 rounded-full px-2 py-0.5 text-[11px] font-semibold ${k.disponible === false || (k.stock ?? 1) <= 0 ? "bg-destructive/10 text-destructive" : (k.stock ?? 99) <= 3 ? "bg-brand-cream text-brand-ink" : "bg-secondary text-secondary-foreground"}`}>{compLabel(k)}</span>}</span>
              <span className="tabular-nums">{formatFcfa(k.price)}</span>
              <Button variant="ghost" size="icon" aria-label="Modifier" onClick={() => setCompForm({ id: k.id, name: k.name, price: String(k.price), stock: k.stock === null || k.stock === undefined ? "" : String(k.stock), disponible: k.disponible !== false })}><IconPencil className="h-4 w-4" /></Button>
        ...
        <DialogContent>
          ...
          {compForm && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><Label>Quantité du jour</Label><Input type="number" min={0} step={1} placeholder="Illimité" value={compForm.stock} onChange={(e) => setCompForm({ ...compForm, stock: e.target.value })} /><p className="text-[11px] text-muted-foreground">Vide = illimité.</p></div>
              <label className="flex items-center gap-2 pt-6 text-sm"><Switch checked={compForm.disponible} onCheckedChange={(v) => setCompForm({ ...compForm, disponible: v })} />Disponible</label>
            </div>
          )}
          <DialogFooter>
